...
Les variables  `embolon`, `oppositon`, `decinveston` & `novonoon` sont vides & ne seront pas utiliser dans la suite de l'analyse. 
...
tt |> 
  ...
  tbl_summary( statistic = stt,             value = ouinon) |> 
  modify_header(label ~ " ") |>
bold_labels() |>
...
Chez les patients à troponine élevée la copeptine ne semble pas apportée grand chose, en tout cas il est impossible de fixer un seuil. Par contre chez les patients à troponine normale l'AUC atteint des valeurs (juste) acceptables avec un seuil quelque part entre 8 & 18. À noter qu'un certain nombre de patients avait une troponine élevée pour d'autres raisons.
...
aa <- rbind(aa, c("Troponine",round(zz[[4]],2),llt))
#
zz <- cor.test(tt$copepth0,tt$doulprev)
llc <- beaup(zz$p.value,affp= FALSE)
aa <- rbind(aa, c("Copeptine",round(zz[[4]],2),llc))
#
zz <- as_tibble(aa)
...
```{r}
#| label: fig-delaitpcp
#| fig-cap:  Troponine à H0 selon le délai entre 1superscript^e^ douleur et prélèvements
...
tt |> 
  ...
  labs(title = "Troponine à H0 selon le délai entre douleur et prélèvements",
...
```{r}
#| label: fig-delaicop
#| fig-cap:  Copeptine à H0 selon le délai entre 1superscript^e^ douleur et prélèvements
...
tt |> 
  ...
  scale_y_continuous(trans='log10') +
  geom_smooth(method  = "lm") +
  labs(title = "Copeptine à H0 selon le délai entre douleur et prélèvements",
