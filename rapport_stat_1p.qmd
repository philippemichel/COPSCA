---
title: "COP-SCA"
subtitle: "Rapport Statistique - V 1.3"
...
expx <- TRUE
classeur <- "copsca1_4.xls"
if (expx){system(paste0("rm -f ",classeur))}
#
cat <- list(all_categorical() ~ "{n}/{N} ({p})")
```
...
patho |>
  left_join(finet, by = "subjid") |>
  drop_na(scanonston) |>
  dplyr::select(6, 7, 10:15, scanonston) |>
  tbl_summary(
    missing = "no",
    by = scanonston,
    type = list(doulh0 ~ "continuous", doulh3 ~ "continuous"),
    statistic = cat
  ) |>
  modify_header(label ~ " ") |>
   modify_spanning_header(c("stat_1","stat_2")~ "**SCA ST-**") |> 
...
zz <- patho |>
    left_join(finet, by = "subjid") |>
  drop_na(scanonston) |>
dplyr::select(16:22,scanonston) |>
mutate(ainson = factor(ainson, levels = c("Non", "Oui")),
       paracetcodon = factor(paracetcodon, levels = c("Non", "Oui"))) 
...
zz <- patho |>
  left_join(finet, by = "subjid") |>
  left_join(tt, by = "subjid") |>
  drop_na(scanonston.x)

hsympt <- as.numeric(dmy_hms(paste0(zz$symptodte, " ", zz$symptohr)))
hurg <- as.numeric(dmy_hms(paste0(patho$urgencdte, " ", patho$urgenhr)))
h0 <- as.numeric(dmy_hms(paste0(zz$prelh0dte, " ", zz$prelh0hr)))
h3 <- as.numeric(dmy_hms(paste0(zz$prelh3dte, " ", zz$prelh3hr)))
#
sympth0 <- (h0 - hsympt) / 60
sympturg <- (hurg - hsympt) / 60
urg0 <- (h0 - hurg) / 60
urg3 <- (h3 - hurg) / 60

var_label(sympth0) <- "Symptômes - Évaluation h0"
var_label(sympturg) <- "Symptômes - Admission urgences"
var_label(urg0) <- "Admission urgences - Évaluation h0"
var_label(urg3) <- "Admission urgences - Évaluation h3"
...
zz |>
  ...
  dplyr::select(70:73, scanonston.x) |>
  tbl_summary(missing = "no",
              by = scanonston.x,
              statistic = cat) |>
  modify_header(label ~ "**Délai (min)**") |>
  add_overall() |>
  modify_spanning_header(c("stat_1", "stat_2") ~ "**SCA ST-**") |>
  add_p() |>
  bold_labels() |>
...
geom_vline(aes(xintercept = 15.6), linetype = "dotdash", color = "red", alpha = 0.6) +
geom_vline(aes(xintercept = 34.2), linetype = "dotdash", color = "blue", alpha = 0.6) +
theme_light() +
...
tt |> 
ggplot() +
aes(x =sex, y = tropoh0, fill = sex) +
  scale_y_continuous(trans='log10') +
geom_violin() +
geom_boxplot(width = 0.2, fill = "grey90") +
  geom_hline(aes(yintercept = 15.6), linetype = "dotdash", color = "red", alpha = 0.6) +
geom_hline(aes(yintercept = 34.2), linetype = "dotdash", color = "blue", alpha = 0.6) +
  theme_light() +
...
geom_density(alpha = 0.5) +
scale_x_continuous(trans='log10') +
geom_vline(aes(xintercept = 10), linetype = "dotdash", color = "black", alpha = 0.6)  +
theme_light() +
...
Le daignostic final a été SCAS ST- pour 10 patients.
...
geom_density(alpha = 0.5) +
geom_vline(aes(xintercept = 15.6), linetype = "dotdash", color = "red") +
geom_vline(aes(xintercept = 34.2), linetype = "dotdash", color = "blue") +
scale_x_continuous(trans='log10') +
theme_light() +
...
```{r}
#| label: fig-ssbtropo0b
#| fig-cap: tropo H0 selon le diagnostic final
tt |> 
ggplot() +
aes(x =scanonston, y = tropoh0, fill = sex) +
  scale_y_continuous(trans='log10') +
geom_boxplot() +
  geom_hline(aes(yintercept = 15.6), linetype = "dotdash", color = "red", alpha = 0.6) +
geom_hline(aes(yintercept = 34.2), linetype = "dotdash", color = "blue", alpha = 0.6) +
  theme_light() +
 scale_x_discrete(labels = c("Oui", "Non")) +
    labs(title = "Troponine H0 selon le diagnostic final",
        x = "SCA non ST -",
y = "pg/mL", 
)+ 
...
geom_density(alpha = 0.5) +
geom_vline(aes(xintercept = 10), linetype = "dotdash", color = "black") +
scale_x_continuous(trans='log10') +
theme_light() +
...
tt |> 
ggplot() +
aes(x =scanonston, y = copepth0, fill = scanonston) +
  scale_y_continuous(trans='log10') +
geom_boxplot() +
  geom_hline(aes(yintercept = 10), linetype = "dotdash", color = "black", alpha = 0.6)  +
  theme_light() +
 scale_x_discrete(labels = c("Oui", "Non")) +
    labs(title = "Copeptine H0 selon le diagnostic final",
        x = "SCA non ST -",
...
::: {.callout-important}
Pour toutes les figures comportant des taux de Troponine ou de copeptine les axes des ordonnées sont en échelle logarithmique.

Pour les graphiques en densité (@fig-tropoh0 par ex.), l'échelle des ordonnées est arbitraire & a donc été omise.
:::
...
Un  test exact de Fisher a été utilisé pour les comparaisons entre données catégorielles en particulier pour les comparaisons des sensiblités, spécifictésn VPP & VPN. Les données numériques ont été comparées par le test de Wilcoxon. Les corrélations ont été testées par la méthode des rangs de Spearman.
